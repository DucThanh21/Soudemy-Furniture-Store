...
// --- NEW: safe endpoint (đổi từ admin_products.php sang manage_products.php) ---
const PRODUCTS_ENDPOINT = '/Do_an/backend/manage_products.php';
...
document.querySelectorAll('.edit-product').forEach(button => {
    button.addEventListener('click', function() {
        const productData = {
            ...
            name: decodeHtml(this.dataset.productName || ''),
            category: this.dataset.productCategory || '',
            price: this.dataset.productPrice || '',
            stock: this.dataset.productStock || '',
            description: decodeHtml(this.dataset.productDescription || ''),
...
const productCurrentImagePreview = document.getElementById('productCurrentImagePreview');
const productSubmitBtn = document.getElementById('productSubmitBtn');

function openProductModal(productData = null) {
    const modalTitle = document.getElementById('productModalTitle');
    const hiddenIdInput = productForm?.querySelector('input[name="id"]');
    if (!productForm || !modalTitle || !hiddenIdInput) return;
    ...
    productImageLabel.textContent = 'Hình ảnh sản phẩm *';
    productCurrentImage.style.display = 'none';
    productCurrentImagePreview.src = '';
    ...
    if (productData && productData.id) {
        ...
        productForm.querySelector('[name="stock"]').value = productData.stock || '';
        productForm.querySelector('[name="description"]').value = productData.description || '';

        productImageInput.value = '';
        productImageInput.required = false;
        productImageLabel.textContent = 'Hình ảnh sản phẩm (tùy chọn)';

        if (productData.image) {
            const normalizedImage = productData.image.startsWith('http') ? productData.image : `../frontend/${productData.image}`;
            productCurrentImagePreview.src = normalizedImage;
            productCurrentImage.style.display = 'block';
        } else {
            productCurrentImage.style.display = 'none';
        }

        productSubmitBtn.textContent = 'Sửa sản phẩm';
...
if (productForm) {
    productForm.addEventListener('submit', function(e) {
        ...
        const imageInput = this.querySelector('[name="image"]');
        const image = imageInput?.files?.[0];
        const isEdit = Boolean(this.querySelector('[name="id"]').value);
