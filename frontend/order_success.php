...
// Reset cart indicators so the header stops showing old items
$_SESSION['cart'] = [];
...
// Đánh dấu đơn hàng đã hoàn tất và lấy danh sách sản phẩm đã mua
$orderedProducts = [];
if (!empty($order_id) && $db) {
    try {
        $updateStmt = $db->prepare("UPDATE orders SET status = 'completed' WHERE id = ?");
        $updateStmt->execute([$order_id]);
    } catch (Exception $e) {
        error_log('order_success update status failed: ' . $e->getMessage());
    }
    ...
    try {
        ...
        $orderedProducts = $itemsStmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
        $orderedProducts = [];
        error_log('order_success products fetch failed: ' . $e->getMessage());
    }
}
...
<section class="order-success-section">
    <div class="container">
        ...
        <div class="order-products">
            <h3>Sản phẩm đã giao dịch</h3>
            <?php if (!empty($orderedProducts)): ?>
            <div class="order-product-grid">
                <?php foreach ($orderedProducts as $prod): 
                    $quantity = (int)$prod['quantity'];
                    $unitPrice = (float)$prod['price'];
                    $lineTotal = $quantity * $unitPrice;
                    $imageSrc = buildImageUrl($prod['product_image'] ?? '');
                ?>
                <article class="order-product-card">
                    <div class="product-thumb">
                        <?php if ($imageSrc): ?>
                            <img src="<?php echo htmlspecialchars($imageSrc); ?>" alt="<?php echo htmlspecialchars($prod['product_name'] ?? 'Sản phẩm'); ?>">
                        <?php else: ?>
                    ...
                    <div class="product-meta">
                        <h4><?php echo htmlspecialchars($prod['product_name'] ?? 'Sản phẩm'); ?></h4>
                        <p>Số lượng: <?php echo $quantity; ?></p>
                        <p class="price">Đơn giá: $<?php echo number_format($unitPrice, 2); ?></p>
                    </div>
            ...
            <?php else: ?>
            <p class="empty-products">Chưa có sản phẩm nào được ghi lại cho đơn hàng này.</p>
            <?php endif; ?>
        </div>
